"""
Real-time multi-agent discussion module for AI-to-AI Feedback API
"""
...
from .database import Agent, Session, get_db
from .providers.factory import get_model_provider
from .utils.feedback_parser import StreamingFeedbackParser
...
# Create router
router = APIRouter(prefix="/realtime-discussion", tags=["Real-time Discussion"])

# Store active discussions
active_discussions: Dict[str, Dict] = {}

class DiscussionParticipant:
    """Class representing a participant in a real-time discussion"""
...
class DiscussionManager:
    """Manager for real-time discussions"""
    ...
    async def get_agent_response(self, agent_id: str, prompt: str) -> str:
...
You are participating in a real-time discussion with multiple AI agents. Each agent has a unique perspective and role.
Claude (an AI assistant) acts as the chair of this discussion, orchestrating the conversation, but you should actively engage with other agents directly.
...
"""
            ...
            # Get response
            response = await provider.generate_completion(system_prompt, prompt)
...
async def create_discussion(
    ...
    """
    Create a new real-time discussion session with multiple agents.
    """
    try:
        # Extract request parameters
        num_agents = request.get("num_agents", 2)
        title = request.get("title", "Real-time Discussion")
        system_prompt = request.get("system_prompt", "You are participating in a real-time discussion with multiple AI agents.")
        tags = request.get("tags", [])
        agent_names = request.get("agent_names")
        agent_roles = request.get("agent_roles")
        agent_models = request.get("agent_models")
        ...
        if num_agents > 10:
            raise HTTPException(status_code=400, detail="Maximum 10 agents are allowed")
        ...
        manager = DiscussionManager(session_id, system_prompt)
        ...
        logger.info(f"Created real-time discussion {session_id} with {num_agents} agents")
        return {"session_id": session_id}
    except Exception as e:
        logger.error(f"Error creating real-time discussion: {e}")
        raise HTTPException(status_code=500, detail=f"Error creating real-time discussion: {str(e)}")
...
async def send_message(
    ...
    """
    Send a message to a real-time discussion.
    """
    try:
        # Extract request parameters
        ...
        for participant in participants:
            ...
            asyncio.create_task(
                manager.get_agent_response(participant.agent_id, message)
            )
        ...
        logger.error(f"Error sending message to real-time discussion: {e}")
...
async def stream_discussion(
    ...
    """
    Stream messages from a real-time discussion.
    """
    try:
        # Get discussion manager
        manager = discussion_managers.get(session_id)
