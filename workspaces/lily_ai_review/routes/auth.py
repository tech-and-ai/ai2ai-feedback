...
async def register_page(request: Request):
    ...
    # The CSRF middleware will handle setting the token in the cookie and context
...
@router.get("/verify")
async def verify_email(request: Request, token: str = None, type: str = None, redirect_to: str = None):
    """
    Handle email verification.

    This route is called by Supabase when a user clicks the verification link in their email.
    The URL format is: /auth/verify?token=xxx&type=email&redirect_to=xxx
    """
    # Log all query parameters for debugging
    logger.info(f"Verify email accessed with query params: {request.query_params}")

    # Even if token is missing, we'll show the verification success page
    # This is because Supabase handles the verification automatically via the URL
    # and the token might be processed before our route handler is called
    try:
        if token:
            # Log the verification attempt if token is present
            logger.info(f"Email verification with token: {token[:10]}...")
        else:
            logger.info("Email verification without token parameter (likely already processed by Supabase)")
