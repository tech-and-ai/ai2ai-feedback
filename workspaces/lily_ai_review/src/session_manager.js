...
class Session {
   ...
   * @param {string} role - The role of the participant (user, assistant, or system)
  ...
  /**
   * Get the session history formatted for OpenAI API
   * @returns {Array} The formatted session history
   */
  getFormattedHistory() {
...
class SessionManager {
  ...
  cleanupExpiredSessions() {
    ...
    if (cleanedCount > 0) {
      console.log(`Cleaned up ${cleanedCount} expired sessions. ${this.sessions.size} active sessions remaining.`);
    }
  }
