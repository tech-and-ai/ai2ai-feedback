...
class NotificationService:
    ...
    async def _get_user_data(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.db.from_('users_view').select('*').eq('id', user_id).execute()

            if response.data and len(response.data) > 0:
                user = response.data[0]

                # Extract user metadata from raw_user_meta_data
                ...
                return {
                    'id': user['id'],
                    'email': user['email'],
                    'username': user_metadata.get('username', user['email'].split('@')[0]),
                    'first_name': user_metadata.get('first_name', ''),
                    'last_name': user_metadata.get('last_name', ''),
                    'full_name': f"{user_metadata.get('first_name', '')} {user_metadata.get('last_name', '')}".strip(),
...
<body>
    ...
    def _get_default_smtp_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'username': os.environ.get('SMTP_USER', 'sales@data4u.uk'),
            'password': os.environ.get('SMTP_PASSWORD', ''),
            'sender_email': os.environ.get('EMAIL_SENDER', 'sales@data4u.uk'),
